"""Point Card Points Report window."""
...
from .service import PointCardPointsService


class PointCardPointsView(ctk.CTkFrame):
    TITLE = "Point Card Points"

    def __init__(self, master, database: Database, session: AppSession):
        super().__init__(master, fg_color="transparent")
        self.service = PointCardPointsService(database, rlevel=getattr(session, "gilevel", 1))
        ...
        self.grid_widget = DataGrid(self, columns=[("code", "Customer", 110), ("name", "Name", 160), ("pcardname", "Card", 110),
                                    ("samt", "Sales Amt", 120), ("spoints", "Points", 100), ("clpoints", "Closing", 100)], key_field="_k")
    ...
    def _show(self):
        try:
            ...
            self.grid_widget.set_rows([{"_k": i, "code": r["code"], "name": r["name"], "pcardname": r["pcardname"],
                                        "samt": f'{r["samt"]:.2f}', "spoints": f'{r["spoints"]}', "clpoints": f'{r["clpoints"]}'}
                                       for i, r in enumerate(res["rows"])])
            t = res["totals"]
            self.summary.configure(text=f'{t["count"]} row(s)  ·  Points {t["spoints"]}', text_color=("gray20", "gray80"))
