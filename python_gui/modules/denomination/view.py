"""Denomination Master window (code / name / value)."""
...
from .service import DenominationError, DenominationService


class DenominationView(ctk.CTkFrame):
    TITLE = "Denomination Master"

    def __init__(self, master, database: Database, session: AppSession):
        super().__init__(master, fg_color="transparent")
        self.service = DenominationService(DenominationRepo(database), session)
        ...
        self.grid_widget = DataGrid(self, columns=[("code", "Code", 100), ("name", "Name", 180),
                                    ("cvalue", "Value", 100)], on_select=self._on_select, key_field="code")
        ...
        ctk.CTkLabel(form, text="Value").grid(row=4, column=0, sticky="w", padx=10)
        self.cvalue = ctk.CTkEntry(form, width=120); self.cvalue.grid(row=5, column=0, padx=10, pady=(0, 6), sticky="w")
    ...
    def _on_select(self, row):
        ...
        self._set(self.name, row.get("name")); self._set(self.cvalue, row.get("cvalue") or 0)
    ...
    def _new(self):
        self._set(self.code, ""); self._set(self.name, ""); self._set(self.cvalue, "0")
        self.grid_widget.clear_selection()

    def _save(self):
        try:
            self.service.save(self.code.get(), self.name.get(), self.cvalue.get() or 0)
        except (DenominationError, Exception) as exc:
