...
from .service import RemakeIssueMemoError, RemakeIssueMemoService

_FIELDS = [("itemcode", "Item"), ("qty", "Qty"), ("weight", "Weight"), ("stonewgt", "Stone"), ("complaint", "Remark")]


class RemakeIssueMemoView(ctk.CTkFrame):
    TITLE = "Remake Issue Memo"

    def __init__(self, master, database: Database, session: AppSession):
        super().__init__(master, fg_color="transparent")
        self.service = RemakeIssueMemoService(PostingEngine(database), session)
    ...
    def _save(self):
        ...
        try:
            res = self.service.save(custcode=self.cust.get(), rows=self._rows, tdate=date.today().isoformat())
        except (RemakeIssueMemoError, Exception) as exc:
