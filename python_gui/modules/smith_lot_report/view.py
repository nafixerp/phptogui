...
from .service import SmithLotReportService


class SmithLotReportView(ctk.CTkFrame):
    TITLE = "Smith Lot Report"

    def __init__(self, master, database: Database, session: AppSession):
        super().__init__(master, fg_color="transparent")
        self.service = SmithLotReportService(database, rlevel=getattr(session, "gilevel", 1))
        ...
        self.grid_widget = DataGrid(self, columns=[
            ("name", "Smith", 180), ("lotno", "Lot No", 100), ("issuewgt", "Issued", 110),
            ("rcvdwgt", "Received", 110), ("pendwgt", "Pending", 110), ("pendqty", "Pend Qty", 90)], key_field="_k")
    ...
    def _show(self):
        try:
            ...
            self.grid_widget.set_rows([{
                "_k": i, "name": r["name"], "lotno": r["lotno"], "issuewgt": f'{r["issuewgt"]:.3f}',
                "rcvdwgt": f'{r["rcvdwgt"]:.3f}', "pendwgt": f'{r["pendwgt"]:.3f}', "pendqty": r["pendqty"]}
                for i, r in enumerate(rows)])
            self.summary.configure(text=f"{len(rows)} lot(s).", text_color=("gray20", "gray80"))
