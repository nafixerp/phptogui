...
from .service import PdcReportService

_RP = {"All": "", "Receipt": "R", "Payment": "P"}


class PdcReportView(ctk.CTkFrame):
    TITLE = "PDC Report"

    def __init__(self, master, database: Database, session: AppSession):
        super().__init__(master, fg_color="transparent")
        self.service = PdcReportService(database, gilevel=getattr(session, "gilevel", 1))
        ...
        self.rp = ctk.CTkOptionMenu(head, width=100, values=list(_RP)); self.rp.pack(side="left", padx=8)
        ...
        self.grid_widget = DataGrid(self, columns=[
            ("tdate", "Date", 100), ("chqdate", "Chq Date", 100), ("partyname", "Party", 160),
            ("bankname", "Bank", 130), ("chqno", "Chq No", 100), ("receipt", "Receipt", 110), ("payment", "Payment", 110)], key_field="slno")
        self.grid_widget.grid(row=2, column=0, sticky="nsew", padx=12, pady=6)
        self._show()

    def _show(self):
        try:
            rows = self.service.report(self.d1.get(), self.d2.get(), rp=_RP[self.rp.get()])
            self.grid_widget.set_rows([{
                "slno": r["slno"], "tdate": r["tdate"], "chqdate": r["chqdate"], "partyname": r["partyname"],
                "bankname": r["bankname"], "chqno": r["chqno"], "receipt": f'{r["receipt"]:.2f}',
                "payment": f'{r["payment"]:.2f}'} for r in rows])
            self.summary.configure(text=f"{len(rows)} cheque(s).", text_color=("gray20", "gray80"))
