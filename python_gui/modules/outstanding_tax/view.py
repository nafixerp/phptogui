...
from .service import OutstandingTaxService


class OutstandingTaxView(ctk.CTkFrame):
    TITLE = "Outstanding Tax Report"

    def __init__(self, master, database: Database, session: AppSession):
        super().__init__(master, fg_color="transparent")
        self.service = OutstandingTaxService(database, rlevel=getattr(session, "gilevel", 1))
        ...
        self.grid_widget = DataGrid(self, columns=[
            ("description", "Description", 240), ("inputtax", "Input Tax", 150), ("outputtax", "Output Tax", 150)], key_field="description")
    ...
    def _show(self):
        try:
            ...
            self.grid_widget.set_rows([{"description": r["description"], "inputtax": f'{r["inputtax"]:.2f}',
                                        "outputtax": f'{r["outputtax"]:.2f}'} for r in res["rows"]])
            self.summary.configure(
                text=f'Output {res["total_output"]:.2f}  ·  Input {res["total_input"]:.2f}  ·  Net {res["net"]:.2f} ({res["net_label"]})',
