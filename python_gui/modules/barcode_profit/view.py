...
from .service import BarcodeProfitService


class BarcodeProfitView(ctk.CTkFrame):
    TITLE = "Barcode Profit Report"

    def __init__(self, master, database: Database, session: AppSession):
        super().__init__(master, fg_color="transparent")
        self.service = BarcodeProfitService(database, rlevel=getattr(session, "gilevel", 1))
        ...
        self.grid_widget = DataGrid(self, columns=[
            ("tdate", "Date", 100), ("billno", "Bill", 90), ("itemname", "Item", 160),
            ("weight", "Wgt", 90), ("amount", "Amount", 110), ("costamt", "Cost", 110),
            ("profit", "Profit", 110), ("stprofit", "St.Profit", 100)], key_field="bcode")
    ...
    def _show(self):
        try:
            ...
            self.grid_widget.set_rows([{
                "bcode": r["bcode"], "tdate": r["tdate"], "billno": r["billno"], "itemname": r["itemname"],
                "weight": f'{r["weight"]:.3f}', "amount": f'{r["amount"]:.2f}', "costamt": f'{r["costamt"]:.2f}',
                "profit": f'{r["profit"]:.2f}', "stprofit": f'{r["stprofit"]:.2f}'} for r in res["rows"]])
            t = res["totals"]
            self.summary.configure(text=f'{t["count"]} line(s)  ·  Amount {t["amount"]:.2f}  ·  Profit {t["profit"]:.2f}',
