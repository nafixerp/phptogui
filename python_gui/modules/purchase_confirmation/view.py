"""Purchase Bill Confirmation window."""
...
from .service import ConfirmError, PurchaseConfirmationService


class PurchaseConfirmationView(ctk.CTkFrame):
    TITLE = "Purchase Bill Confirmation"

    def __init__(self, master, database: Database, session: AppSession):
        super().__init__(master, fg_color="transparent")
        self.service = PurchaseConfirmationService(database, session)
        self._sel = None
        ...
        self.grid_widget = DataGrid(self, columns=[("docno", "Doc No", 100), ("billno", "Bill No", 100), ("tdate", "Date", 100),
                                    ("name", "Supplier", 180), ("netamt", "Net Amt", 110), ("balance", "Balance", 110)],
    ...
    def _reload(self):
        try:
            ...
            self.grid_widget.set_rows([{"slno": r["slno"], "docno": str(r.get("docno") or ""), "billno": str(r.get("billno") or ""),
                                        "tdate": str(r.get("tdate") or ""), "name": str(r.get("name") or ""),
                                        "netamt": f'{float(r.get("netamt") or 0):.2f}', "balance": f'{float(r.get("balance") or 0):.2f}'} for r in rows])
    ...
    def _confirm(self):
        ...
        try:
            msg = self.service.confirm(self._sel)
