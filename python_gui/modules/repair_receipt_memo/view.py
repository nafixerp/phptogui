...
from .service import RepairReceiptMemoError, RepairReceiptMemoService

_FIELDS = [("itemcode", "Item"), ("qty", "Qty"), ("weight", "Weight"), ("stonewgt", "Stone"), ("complaint", "Complaint")]


class RepairReceiptMemoView(ctk.CTkFrame):
    TITLE = "Repair Receipt Memo"

    def __init__(self, master, database: Database, session: AppSession):
        super().__init__(master, fg_color="transparent")
        self.service = RepairReceiptMemoService(PostingEngine(database), session)
        ...
        ctk.CTkLabel(head, text="Cust").pack(side="left", padx=(16, 4))
        self.cust = ctk.CTkEntry(head, width=100); self.cust.pack(side="left")
        ctk.CTkLabel(head, text="Advance").pack(side="left", padx=(8, 4))
        self.recvamt = ctk.CTkEntry(head, width=90); self.recvamt.pack(side="left"); self.recvamt.insert(0, "0")
        ctk.CTkButton(head, text="Save Memo", width=100, command=self._save).pack(side="right", padx=6)
        ...
        self.grid_widget = DataGrid(self, columns=[(k, lbl, 100) for k, lbl in _FIELDS], key_field="itemcode")
    ...
    def _save(self):
        ...
        try:
            res = self.service.save(custcode=self.cust.get(), rows=self._rows, recvamt=self.recvamt.get() or 0,
                                    tdate=date.today().isoformat())
        except (RepairReceiptMemoError, Exception) as exc:
            self.status.configure(text=str(exc).splitlines()[0], text_color="#C0392B"); return
        self._rows = []; self.grid_widget.set_rows([])
        self.status.configure(text=f'Saved {res["bill_no"]} (slno {res["slno"]})', text_color="#1E8449")
