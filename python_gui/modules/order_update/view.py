"""Order Update window — create / delete jewellery orders."""
...
from .service import OrderUpdateError, OrderUpdateService

_ITEM_FIELDS = [("code", "Item"), ("qty", "Qty"), ("weight", "Weight"), ("mcharge", "MC"), ("amount", "Amount")]


class OrderUpdateView(ctk.CTkFrame):
    TITLE = "Order Update"

    def __init__(self, master, database: Database, session: AppSession):
        super().__init__(master, fg_color="transparent")
        self.service = OrderUpdateService(PostingEngine(database), session)
        ...
        ctk.CTkButton(head, text="Save Order", width=110, command=self._save).pack(side="right", padx=6)
        hdr = ctk.CTkFrame(self, fg_color="transparent"); hdr.grid(row=1, column=0, sticky="ew", padx=12, pady=4)
        self.order: dict[str, ctk.CTkEntry] = {}
        for key, label, dflt in [("jewlcode", "Jewl Code", ""), ("custcode", "Cust Code", ""),
                                 ("custname", "Customer", ""), ("smcode", "Salesman", ""),
                                 ("rate", "Rate", "0"), ("duedate", "Due Date", "")]:
            ctk.CTkLabel(hdr, text=label).pack(side="left", padx=(6, 2))
            e = ctk.CTkEntry(hdr, width=100); e.pack(side="left")
            if dflt:
                e.insert(0, dflt)
            self.order[key] = e
        ...
        self.grid_widget = DataGrid(self, columns=[(k, lbl, 100) for k, lbl in _ITEM_FIELDS], key_field="code")
    ...
    def _save(self):
        order = {k: e.get() for k, e in self.order.items()}
        order["tdate"] = date.today().isoformat()
        try:
            res = self.service.save(order, self._items)
        except (OrderUpdateError, Exception) as exc:
            self.status.configure(text=str(exc).splitlines()[0], text_color="#C0392B"); return
        self._items = []; self.grid_widget.set_rows([])
        self.status.configure(text=f'Saved {res["ordno"]} (slno {res["slno"]}, {res["items"]} item(s))', text_color="#1E8449")
