...
from .service import OrderProfitService


class OrderProfitView(ctk.CTkFrame):
    TITLE = "Order Profit Analysis"

    def __init__(self, master, database: Database, session: AppSession):
        super().__init__(master, fg_color="transparent")
        self.service = OrderProfitService(database, gilevel=getattr(session, "gilevel", 1))
        ...
        self.grid_widget = DataGrid(self, columns=[
            ("tdate", "Date", 100), ("billno", "Bill", 90), ("orderno", "Order", 100), ("custname", "Customer", 150),
            ("totadvamt", "Adv Amt", 110), ("totsoldamt", "Sold Amt", 110), ("diff", "Diff", 110)], key_field="_k")
    ...
    def _show(self):
        try:
            ...
            self.grid_widget.set_rows([{
                "_k": i, "tdate": r["tdate"], "billno": r["billno"], "orderno": r["orderno"], "custname": r["custname"],
                "totadvamt": f'{r["totadvamt"]:.2f}', "totsoldamt": f'{r["totsoldamt"]:.2f}', "diff": f'{r["diff"]:.2f}'}
                for i, r in enumerate(res["rows"])])
            t = res["totals"]
            self.summary.configure(text=f'{t["count"]} order(s)  ·  Net diff {t["diff"]:.2f}', text_color=("gray20", "gray80"))
