...
from .service import TdsReportService


class TdsReportView(ctk.CTkFrame):
    TITLE = "TDS Report"

    def __init__(self, master, database: Database, session: AppSession):
        super().__init__(master, fg_color="transparent")
        self.service = TdsReportService(database, rlevel=getattr(session, "gilevel", 1))
        ...
        self.grid_widget = DataGrid(self, columns=[
            ("tdate", "Date", 100), ("docno", "Doc No", 100), ("name", "Smith", 180),
            ("tmcharge", "MC Amount", 120), ("tdsperc", "TDS %", 80), ("tdsamt", "TDS Amt", 120)], key_field="_k")
    ...
    def _show(self):
        try:
            ...
            self.grid_widget.set_rows([{
                "_k": i, "tdate": r["tdate"], "docno": r["docno"], "name": r["name"],
                "tmcharge": f'{r["tmcharge"]:.2f}', "tdsperc": f'{r["tdsperc"]:.2f}', "tdsamt": f'{r["tdsamt"]:.2f}'}
                for i, r in enumerate(res["rows"])])
            t = res["totals"]
            self.summary.configure(text=f'{len(res["rows"])} row(s)  ·  TDS {t["tdsamt"]:.2f}', text_color=("gray20", "gray80"))
