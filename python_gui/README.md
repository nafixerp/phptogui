...
| 6 | **Diamond Purchase** (register + stone detail + **save**) | `DiamondPurchaseBillController` | ✅ done — register/load (`pr='P' dmd='Y'` over `purchasem`/`purchased` + `purchased_dmddet` grouped by `prow`) **and save** (`DiamondPurchaseSaveService`: purchasem header + purchased rows + stone sub-rows, item-stock increase via `core/stock_adjust`, `DPBPREF`+`DPURCHASEB` doc-no, PL daybook via `PurchasePostingService.build_entries` with ROUND; zero-sum + stock tested) |
...
| 9 | **Kuri Type Master + PDC Report + Smith Lot Report + Gold Loan (load + save)** | `KuriTypeMasterController`, `PdcReportController`, `SmithLotReportController`, `GoldLoanController` | ✅ done (kuritype replace/column-filter + in-use guard; pdclist receipt/payment split + delete; smith lot issued/received/pending; Gold Loan load **and save** — `GoldLoanSaveService` creates/updates loan + loan_items and records loancolln repayments that recompute balance/status, all column-filtered since the demo `loan` schema differs from the live one; tested) |
